$(function(){
    ...
    $('.confirm').on('click',function(){
        var confirmation = confirm("Jeni i sigurte?");
        if(!confirmation){
            return false;   
        }
    });
    ...
    $('#login').on('click',function(){
        ...
        $.post('cart/data.php?q=verify',
               ...
               function(data){
                    if(data == 1){                        
                        $('.error').removeClass().addClass('alert alert-success').html('<i class="fa fa-unlock"></i> Duke u kycur...'); 
                        setInterval(function(){
                            window.location = 'admin.php';
                        },1000);
                    }else{
                        $('.error').addClass('alert alert-danger').html('Useri/Paswordi eshte gabim!');
                        $('#username').val('');
                        $('#password').val(''); 
                    }
               });
